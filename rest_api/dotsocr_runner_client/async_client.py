"""
Asynchronous DotsOCR client implementation.
"""
...
class AsyncDotsOCRRunnerClient:
    """
    Asynchronous client for DotsOCR REST API.
    
    Provides async methods for uploading files, checking task status, retrieving results,
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:8080", 
        ...
        """
        Initialize AsyncDotsOCR client.
        ...
        # Default headers
        self._headers = {
            'User-Agent': 'DotsOCR-Python-Async-Client/1.0.0'
        }
            ...
            async with session.request(method, url, **kwargs) as response:
                ...
                if not response.ok:
                    try:
                        ...
                        message = error_data.get('error', {}).get('message', await response.text())
    ...
    async def upload_pdf(self, pdf_path: str, dpi: Optional[int] = None) -> UploadResponse:
        """
        ...
        try:
            ...
            response_data = await self._make_request('POST', '/api/v1/ocr/pdf/upload', data=data)
    ...
    async def upload_images(self, image_paths: List[str], dpi: Optional[int] = None) -> UploadResponse:
        """
        ...
        try:
            ...
            for path_obj in image_path_objs:
                ...
                data.add_field('files', file_data,
                            filename=filename,
            ...
            response_data = await self._make_request('POST', '/api/v1/ocr/images/upload', data=data)
    ...
    async def get_task_status(self, task_id: str) -> TaskStatusResponse:
        ...
        Raises:
            ...
            APIError: For other API errors
        """
        data = await self._make_request('GET', f'/api/v1/ocr/status/{task_id}')
    ...
    async def get_task_result(self, task_id: str) -> OCRResult:
        """
        ...
        Raises:
            ...
            APIError: For other API errors
        """
        data = await self._make_request('GET', f'/api/v1/ocr/result/{task_id}')
    ...
    async def wait_for_completion(self, task_id: str, poll_interval: float = 2.0,
    ...
    async def delete_task(self, task_id: str) -> DeleteResponse:
        """
        ...
        data = await self._make_request('DELETE', f'/api/v1/ocr/task/{task_id}')
    ...
    async def list_tasks(self) -> TasksListResponse:
        """
        List all tasks.
        ...
        """
        data = await self._make_request('GET', '/api/v1/ocr/tasks')
    ...
    async def list_documents(self, page: int = 1, page_size: int = 10, 
        ...
        data = await self._make_request('GET', '/api/v1/documents', params=params)
    ...
    async def search_documents(self, query: str, scope: SearchScope = SearchScope.FILENAMES,
        ...
        data = await self._make_request('GET', '/api/v1/documents/search', params=params)
    ...
    async def get_document_details(self, document_id: str) -> DocumentDetailsResponse:
        """
        ...
        Raises:
            APIError: For API errors
        """
        data = await self._make_request('GET', f'/api/v1/documents/{document_id}')
        
        try:
            # Parse the flat response structure from the server (same as sync version)
            metadata = data['metadata']
    ...
    async def delete_document(self, document_id: str) -> DeleteDocumentResponse:
        ...
        Raises:
            APIError: For API errors
        """
        data = await self._make_request('DELETE', f'/api/v1/documents/{document_id}')
    ...
    async def export_document(self, document_id: str, export_format: ExportFormat) -> ExportResponse:
        """
        ...
        session = await self._get_session()
        url = f"{self.base_url}/api/v1/documents/{document_id}/export"
    ...
    async def get_document_markdown(self, content_hash: str, document_type: str, file_name: Optional[str] = None,
        ...
        data = await self._make_request('POST', '/api/v1/documents/export', json=request_data)
    ...
    async def export_document_markdown(self, request: MarkdownExportRequest) -> MarkdownExportResponse:
        """
    ...
    async def get_image_binary(self, content_hash: str, id: int, dpi: int) -> ImageBinaryResponse:
        """
        ...
        try:
            async with session.post(full_url, json=request_data) as response:
                # Handle authentication errors
                if response.status == 401:
                    raise AuthenticationError("Authentication failed")
    ...
    async def ocr_on_bbox(self, content_hash: str, page_num: int, dpi: int, bbox: List[int],
        ...
        data = await self._make_request('POST', f'/api/v1/documents/{content_hash}/ocr_on_bbox', json=request_data)
    ...
    async def store_ocr_result(self, content_hash: str, page_num: int, 
        ...
        data = await self._make_request('POST', f'/api/v1/documents/{content_hash}/store_ocr_result', json=request_data)
    ...
    async def complete_ocr(self, content_hash: str) -> CompleteOcrResponse:
        """
        Complete OCR task for a document.
        
        This endpoint marks the OCR task as completed, updating document status
        ...
        Raises:
            APIError: For API errors
        """
        data = await self._make_request('POST', f'/api/v1/documents/{content_hash}/ocr/complete', json={})
    ...
    async def embed_document(self, content_hash: str) -> EmbedDocumentResponse:
        """
        ...
        Raises:
            APIError: For API errors
        """
        data = await self._make_request('POST', f'/api/v1/document/{content_hash}/embedding')
    ...
    async def semantic_query(self, query_text: str, limit: Optional[int] = None,
        ...
        data = await self._make_request('POST', '/api/v1/documents/query/semantic', json=request_data)
...
async def create_async_client(base_url: str = "http://127.0.0.1:8080", 
                           auth_token: str = None, timeout: float = 30.0) -> AsyncDotsOCRRunnerClient:
    """
    Create and return an AsyncDotsOCRRunnerClient instance.
    
    Args:
        base_url: Base URL of DotsOCR server
        auth_token: Authentication token for API access
        timeout: Request timeout in seconds
        
    Returns:
        AsyncDotsOCRRunnerClient instance
    """
    return AsyncDotsOCRRunnerClient(base_url, auth_token, timeout)
